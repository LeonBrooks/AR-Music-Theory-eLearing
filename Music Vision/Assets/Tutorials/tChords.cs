...
public class tChords : Tutorial
{
    public override IEnumerator tutorial()
    {
        ...
        yield return speakAndWait("Basic chords usually have three notes but chords with more or less notes are common as well. " +
        ...
        yield return speakAndWait("So let's take a look at how basic chords are set up and what variations there are. " +
            "The most basic chord has three notes which are each separated by a third.");
        ...
        yield return waitForContinue();
        yield return speakAndWait("Similarly to scales chords can also be in major or minor key. " +
        ...
        yield return waitForContinue();
        yield return speakAndWait("If you weren't quite sure anymore what the difference between a major and a minor third is, here a quick refresher:" +
        ...
        t3 = instantiateTooltip(sheet.gameObject.transform, new Vector3(-3.45f, -0.4f, 0), "C major chord", new Vector3(0.03f, 0.03f, 0.03f));
        t3.GetComponentInChildren<LineRenderer>().enabled = false;
        km.showNameplate(Key.C4.ToString());
        yield return waitForContinue();
        mc.resetAllKeys();
        yield return speakAndWait("For a minor chord this is exactly reversed. The lower two notes are separated by a minor third, " +
        ...
        t4 = instantiateTooltip(sheet.gameObject.transform, new Vector3(-0.6f, -0.4f, 0), "C minor chord", new Vector3(0.03f, 0.03f, 0.03f));
        t4.GetComponentInChildren<LineRenderer>().enabled = false;
        km.changeColor(Color.Red, Key.CS4.ToString());
        km.changeColor(Color.Red, Key.D4.ToString());
        ...
        t1 = instantiateTooltip(sheet.gameObject.transform, new Vector3(-0.49f, 0, 0), "first inversion", new Vector3(0.03f,0.03f,0.03f));
        t1.GetComponentInChildren<LineRenderer>().enabled = false;
        yield return waitForContinue();
        yield return speakAndWait("If you move the second note up as well that is called the second inversion");
        sheet.drawChord(6, notes: new (Key, int)[] { (Key.B4, 0), (Key.E5, 0), (Key.G5, 0) });
        t2 = instantiateTooltip(sheet.gameObject.transform, new Vector3(2.36f, 0, 0), "second inversion", new Vector3(0.03f, 0.03f, 0.03f));
        ...
        yield return speakAndWait("How about changing it into a D major chord instead. Can you do that by dragging the notes to the correct positions?");
        ...
        yield return waitForNoteInput(c1.ToArray(), new Key[] { Key.F4, Key.A4, Key.D5 }, new int[] { 1, 0, 0 }, "Drag the notes to invert the chord once (D major)");
